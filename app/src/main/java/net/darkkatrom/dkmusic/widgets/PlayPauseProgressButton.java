...
public class PlayPauseProgressButton extends FrameLayout {
    ...
    /**
     * Notify that the current song is completely played
     */
    ...
    public void updateState() {
        ...
        int progress = 0;
        if (mPlayBackCompleted && mCurrentSongProgress == 0) {
        ...
        if (mPlayPauseButton.isEnabled() != isEnabled()) {
    ...
    private void postUpdate() {
        if (mUpdateProgress == null) {
            mUpdateProgress = new Runnable() {
                ...
                public void run() {
                    updateState();
                    postDelayed(mUpdateProgress, isDragging() ? UPDATE_FREQUENCY_FAST_MS
                            : UPDATE_FREQUENCY_MS);
                }
            };
        }
